			...
			<Field.Group>
				<div class='grid grid-cols-5 gap-4'>
					<Field.Field class='col-span-4'>
						...
						<Input id='function-name' bind:value={pendingFunc.data.name} disabled={!editing} />
					</Field.Field>
					<Field.Field class='col-span-1'>
						<Field.Label for='function-version'>Version</Field.Label>
						{#if editing}
							<Input id='function-version' bind:value={pendingFunc.data.version} type='number' />
						{:else}
							<Select.Root
								type='single'
								bind:value={version}
				...
				<Field.Field>
					...
					<Textarea
						...
						bind:value={pendingFunc.data.description}
						disabled={!editing}
					/>
				</Field.Field>
				<Field.Field orientation='horizontal'>
					<Checkbox id='function-need-auth' bind:checked={pendingFunc.data.authenticated} disabled={!editing} />
			...
			<Field.Set>
				...
				<Field.Field>
					<div class='bg-muted/40 text-muted-foreground flex items-center gap-2 rounded border p-3 font-mono text-sm'>
						...
						<span>Root Class ID: {pendingFunc.data.root_node.class_id || 'Not Selected'}</span>
					</div>
					{#if editing}
						<UserClassTree
							nodeClass={data.root}
							ref={data.project.reference}
							bind:selectedID={pendingFunc.data.root_node.class_id}
						/>
					{/if}

					{#if pendingFunc.data.root_node.class_id !== ''}
								...
								<div class='grid grid-cols-5'>
									...
									<Field.Field class='col-span-2' orientation='horizontal'>
										<Checkbox
											...
											bind:checked={pendingFunc.data.root_node.check_permission}
											disabled={!editing}
										/>
										<Field.Label for='check-permissions'>Check Permissions</Field.Label>
									</Field.Field>
									<span class='col-span-1 text-sm italic'>Bits {pendingFunc.data.root_node.check_bits}</span>
								</div>
							</Card.Content>
							<Card.Content>
								<div class={!editing ? 'pointer-events-none opacity-70' : ''}>
									<PermissionSelector bind:bits={pendingFunc.data.root_node.check_bits} />
								</div>
							</Card.Content>
						</Card.Root>
					{/if}
			...
			<Field.Set>
				...
				<Field.Field>
					<div class={!editing ? 'pointer-events-none opacity-80 grayscale-[0.3]' : ''}>
						<ClassFuncNodeEditor bind:node={pendingFunc.data.node} />
					</div>
				</Field.Field>
			</Field.Set>
		</Field.Set>
