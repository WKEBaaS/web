	...
	<Field.Group>
		...
		<Field.Set>
			<Field.Group>
				<div class='grid grid-cols-5 gap-4'>
					<Field.Field class='col-span-4'>
						...
						<Input id='function-name' bind:value={createClassFuncInput.data.name} />
					</Field.Field>
					<Field.Field class='col-span-1'>
						<Field.Label for='function-version'>Version</Field.Label>
						<Input id='function-version' bind:value={createClassFuncInput.data.version} type='number' />
					</Field.Field>
				</div>
				<Field.Field>
					<Field.Label for='function-description'>Description</Field.Label>
					<Textarea class='h-32' id='function-description' bind:value={createClassFuncInput.data.description} />
				</Field.Field>
				<Field.Field orientation='horizontal'>
					<Checkbox id='function-need-auth' bind:checked={createClassFuncInput.data.authenticated} />
			...
			<Field.Set>
				...
				<Field.Field>
					<div class='bg-muted/40 text-muted-foreground flex items-center gap-2 rounded border p-3 font-mono text-sm'>
						...
						<span>Root Class ID: {createClassFuncInput.data.root_node.class_id || 'Not Selected'}</span>
					</div>
					<UserClassTree
						nodeClass={data.root}
						ref={data.project.reference}
						bind:selectedID={createClassFuncInput.data.root_node.class_id}
					/>
					{#if createClassFuncInput.data.root_node.class_id !== ''}
						<Card.Root>
								...
								<div class='grid grid-cols-5'>
									...
									<Field.Field class='col-span-2' orientation='horizontal'>
										<Checkbox
											...
											bind:checked={createClassFuncInput.data.root_node.check_permission}
										/>
										<Field.Label for='check-permissions'>Check Permissions</Field.Label>
									</Field.Field>
									<span class='col-span-1 text-sm italic'>Bits {createClassFuncInput.data.root_node.check_bits}</span>
								</div>
							</Card.Content>
							<Card.Content>
								<PermissionSelector bind:bits={createClassFuncInput.data.root_node.check_bits} />
							</Card.Content>
						</Card.Root>
					{/if}
				</Field.Field>
			...
			<Field.Set>
				...
				<Field.Field>
					<ClassFuncNodeEditor bind:node={createClassFuncInput.data.node} />
				</Field.Field>
			</Field.Set>
		</Field.Set>
	</Field.Group>
